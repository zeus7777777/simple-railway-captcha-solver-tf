...
class TrainModel():
    ...
    def _generate_input_pipeline(self, tfrecord_file):
        def _parse(e):
            ...
            return image, features['captcha_ints'][0], features['captcha_ints'][1], features['captcha_ints'][2], features['captcha_ints'][3], features['captcha_ints'][4]
        ...
        self.image, self.captcha_0, self.captcha_1, self.captcha_2, self.captcha_3, self.captcha_4 = iterator.get_next()
    
    def _build_model(self):
        fc1, fc2, fc3, fc4, fc5 = _model(self.image, train=True)
        ...
        self.loss = tf.reduce_mean(loss1 + loss2 + loss3 + loss4 + loss5)
        ...
        self.accu = self.accu1 * self.accu2 * self.accu3 * self.accu4 * self.accu5
    ...
    def add_summary(self, sess, train_loss, train_accu, valid_loss, valid_accu):
        ...
        summary.value.add(tag='Loss/Validation Loss', simple_value=valid_loss)
        summary.value.add(tag='Accuracy/Training Accuracy', simple_value=train_accu)
...
class ValidModel():
    ...
    def _build_model(self):
        fc1, fc2, fc3, fc4, fc5 = _model(self.image, train=False)
...
class TestModel():
    ...
    def predict(self, sess, image):
        a,b,c,d,e = sess.run([self.predict1, self.predict2, self.predict3, self.predict4, self.predict5], feed_dict={self.image:[image]})
        return a,b,c,d,e