...
def main(captcha_file, prefix, tfrecord_path):
    ...
    captcha_files = ['data/56_imitate_vali_set/captcha_vali.csv']
    '''prefix = [
    ...
    prefix = ['data/56_imitate_vali_set/']

    writer = tf.python_io.TFRecordWriter('tfrecord/images_valid_56.tfrecord')
    cnt = 0

    for i in range(len(captcha_files)):
        file_names, captchas = parse_csv_captcha(captcha_files[i])
