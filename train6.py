...
def main():
    ...
    with train_graph.as_default():
        train_model = model.TrainModel(batch_size=BATCH_SIZE, learning_rate=LEARNING_RATE, tfrecord_file='tfrecord/images_train_6.tfrecord')
    with valid_graph.as_default():
        valid_model = model.ValidModel(batch_size=BATCH_SIZE, tfrecord_file='tfrecord/images_valid_6.tfrecord')
    
    train_sess = tf.Session(graph=train_graph)
    valid_sess = tf.Session(graph=valid_graph)

    if LOAD_MODEL:
        train_model.load(train_sess, 'log6')
    else:
    ...
    for it in range(it_start, ITERATION):
        ...
        if (it+1)%VALID_PERIOD==0:
            train_model.save(train_sess, 'log6')
            valid_model.load(valid_sess, 'log6')
            valid_loss = []
            valid_accu = []
            for j in range(10):
